<!doctype html>
<html lang="ru">
<head>
   <meta charset="utf-8">
   <meta http-equiv="X-UA-Compatible" content="IE=edge">
   <meta name="viewport" content="width=device-width, initial-scale=1.0">
   <title>Сделаем лучше вместе!</title>
   <link rel="stylesheet" href="asstes/css/main.css">
   <link rel="stylesheet" href="asstes/css/media.css">
</head>
<body>
   <header class="header">
      <div class="container">
         <div class="header__content">
            <a href="index.php" class="logo">
               <img src="assets/images/logo/logo.png" alt="LOGO" class="logo__img">
            </a>
            <a href="login.php" class="btn">Вход</a>
         </div>
      </div>
   </header>
   <main class="main">
      <section class="section">
         <div class="container">
            <div class="section__heading">
               <h1 class="sectuib__title">Городской портал - Сделаем лучше вместе!</h1>
            </div>
            <div class="section__content">
               <!-- Счётчик -->
               <div class="counter">
                  <span class="counter__name">Количество решенных заявок</span>
               </div>
               <!-- Картинки заявок -->
               <div class="applications">
                  <!-- 1 заявка -->
                  <div class="application">
                     <img src="" alt="after" class="application__img img_after">
                     <img src="" alt="before" class="application__img img_before">
                     <p class="application__name"></p>
                     <div class="inline">
                        <p class="application__category"></p>
                        <p class="application__date"></p>
                     </div>
                  </div>
                  <!-- 2 заявка -->
                  <div class="application">
                     <img src="" alt="after" class="application__img img_after">
                     <img src="" alt="before" class="application__img img_before">
                     <p class="application__name"></p>
                     <div class="inline">
                        <p class="application__category"></p>
                        <p class="application__date"></p>
                     </div>
                  </div>
                  <!-- 3 заявка -->
                  <div class="application">
                     <img src="" alt="after" class="application__img img_after">
                     <img src="" alt="before" class="application__img img_before">
                     <p class="application__name"></p>
                     <div class="inline">
                        <p class="application__category"></p>
                        <p class="application__date"></p>
                     </div>
                  </div>
                  <!-- 4 заявка -->
                  <div class="application">
                     <img src="" alt="after" class="application__img img_after">
                     <img src="" alt="before" class="application__img img_before">
                     <p class="application__name"></p>
                     <div class="inline">
                        <p class="application__category"></p>
                        <p class="application__date"></p>
                     </div>
                  </div>
               </div>
            </div>
         </div>
      </section>
   </main>
   <footer class="footer">
      <div class="container">
         <div class="footer__content">
            <a href="index.php" class="logo">
               <img src="assets/images/logo/logo.png" alt="LOGO" class="logo__img">
            </a>
            <p class="copyright">© Терентьев Кирилл. Все права защищены 2022</p>
         </div>
      </div>
   </footer>
   <script src="assets/js/main.js"></script>
</body>
</html>